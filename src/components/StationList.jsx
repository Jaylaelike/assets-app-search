...
function StationList({ onOptionSelected, onClear }) {
  ...
  const fetchDataFromAPI = async () => {
    try {
      const response = await axios.get(
        import.meta.env.VITE_ASSETS_STATION_LIST
      );
  ...
  const handleOptionSelected = (event, value) => {
    ...
    onOptionSelected(value.Station); // Replace 'column_name' with your actual API response key
  };
